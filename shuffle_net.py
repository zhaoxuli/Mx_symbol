...
def make_stage(data, stage, groups=3):
    ...
    if groups == 1:
        #u can ignore -1  the four block num_filter
        #case the group conv the num_filter must be divided exactly by group_num
    ...
    # The first shuffel block in symbole
...
def get_shufflenet(num_classes=10):
    ...
    data = mx.sym.Pooling(data=data, kernel=(3, 3), pool_type='max',
    	                  stride=(2, 2), pad=(1, 1))
# In this function  the group_num default is 3
# U can modify the stage num get diff block num
    # stage is 2 init the first bolock 24 num_filter do not get other shuffleUnit
    # the featuremap's  size redueced by  half
    data = make_stage(data, 2)
    # stage is 3 the first get concat result the size redueced by half
    # second  do add block
